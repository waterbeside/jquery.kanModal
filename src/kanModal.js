...
;(function($, undefined) {
	...
	$.extend(Modal.prototype, {
		show: function() {
			...
			if (this.options.left !== undefined) {
				$dialog.css({'left': this.options.left});
			}
			...
			if (this.options.top !== undefined) {
				$dialog.css({'top': this.options.top});
			}
					...
					mousemove:function(e){
						if($modal_draggable.length>0){							
							if (dragging) {
						        ...
						        var oX = e.clientX - iX;
						        var oY = e.clientY - iY;
						        $modal_draggable.css({"left":oX + "px", "top":oY + "px"});
						        return false;
					        }
				    	}
					},
	...
	$.fn[pluginName].defaultBtnsGetForm = function(obj){
		...
		if(formLen==1){
			...
			$form1 = obj.$modal.find('.table_edit').find('form');
			if($form1.length>0){
				return $form1;
			}else{
	...
	$.fn[pluginName].createBtn = function(obj,btnSetting){
		...
		if(typeof(opt.onclick)=='function'){
			$returnBtn.click(function(){opt.onclick(this,obj)})
		}
		return $returnBtn;
	};
		...
		if(typeof(obj.options.btns)=="string"){
			...
			$.each(obj.options.btns,function(i,item){
				...
				if(typeof(item)=="string"){
					...
					var btnSetting = $.inArray(btnName,btnDefaultNameArray)<0 ? item : $.extend({},defaultsBtnList[btnName],item) ;
					var $strbtn = $.fn[pluginName].createBtn(obj,btnSetting);
				}
